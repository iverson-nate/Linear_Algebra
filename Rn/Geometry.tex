...
\sin \theta & \cos \theta
\end{bmatrix*}$
and the translation vector is $\mathbal{A}(\vec{0})=\vec{b}$.
\end{proposition}
\begin{proof}
...
$\vec{0}$ is the center of rotation.
...
\item $\dfrac{1}{m^2+1}\begin{bmatrix*}[C] m^2-1 & -2m \\ -2m & 1-m^2 \end{bmatrix*}$ for reflection about $y=mx$.
\end{enumerate}
\end{proposition}
